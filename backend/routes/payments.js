...
async function recordRevenue(reservation, paymentData) {
  ...
  try {
    console.log('💰 Recording revenue for reservation:', reservation._id);

    // Check if invoice already exists
    const existingInvoice = await Invoice.findOne({ reservation: reservation._id });
    if (existingInvoice) {
      console.log('⚠️ Invoice already exists for reservation:', reservation._id);
      return true;
    }
    ...
    console.log('✅ Revenue recorded: Invoice & Payment created', invoice._id);
    return true;
  } catch (error) {
...
async function createRateHawkBooking(reservationParam, orderId) {
  ...
  try {
    ...
    if (bookHash) {
      console.log('✅ Found existing book_hash from prebook step:', bookHash);
      ...
      if (depositAmount && depositCurrency) {
         console.log(`💰 Using pre-booked payment details: ${depositAmount} ${depositCurrency}`);
      }
    } else {
      // Fallback to old flow: Prebook now (if not done before payment)
      console.log('🔄 Step 1: Prebooking (Legacy Flow)...');
    ...
    console.log('💰 Final Payment details for booking:');
    ...
    if (bookingStatus.status === 'ok') {
      ...
      console.log('🔍 Hotel ID:', reservation.hotel.hotelId);
      ...
      if (reservation.hotel.hotelId === 'test_hotel_do_not_book' || reservation.hotel.name === 'Test Hotel Do Not Book') {
        ...
        console.log('⚠️ Test hotel detected - treating as successful sandbox booking');
        await Reservation.findByIdAndUpdate(reservation._id, {
          status: 'confirmed',
      ...
      console.error('❌ RateHawk booking failed (non-test hotel):', bookingStatus.status);
    ...
    console.error('❌ RateHawk booking error:', error.message);
...
router.post('/kashier/create-session', async (req, res) => {
  ...
  if (token) {
    try {
      ...
      if (user) {
        ...
        console.log('👤 Authenticated user for booking:', userId);
      }
    } catch (authError) {
      console.log('⚠️ Token invalid or expired, proceeding as guest booking');
    }
  }
  ...
  try {
    ...
    const reservation = new Reservation({
      ...
      originalPrice: discountAmount ? (totalPrice + (discountAmount || 0)) : totalPrice,
      currency: currency || 'EGP',
      taxData: selectedRate.taxData || [],
    ...
    await reservation.save();
    console.log('📝 Pending reservation created:', reservation._id);
    if (promoCode) {
      console.log(`   🎟️ Promo code attached to reservation: ${promoCode}`);
    ...
    try {
      ...
      console.error('❌ Kashier session creation error (Detailed):', JSON.stringify(paymentError, Object.getOwnPropertyNames(paymentError)));
...
router.post('/kashier/webhook', express.json(), async (req, res) => {
  ...
  try {
    ...
    if (!reservation) {
      console.error('❌ Reservation not found for order:', orderId);
      return res.status(404).json({ error: 'Reservation not found' });
    }

    // Check if already processed
    if (reservation.status === 'confirmed') {
      console.log('⚠️ Reservation already confirmed');
      return res.json({ received: true, processed: true });
    }

    console.log('✅ Payment successful, creating RateHawk booking...');
    ...
    if (reservation.promoCode) {
      try {
        console.log(`🎟️ Processing promo code: ${reservation.promoCode}`);
        ...
        if (promoCodeDoc) {
          console.log(`   ✓ Promo code found in database: ${promoCodeDoc.code}`);
      ...
      try {
        ...
        if (reservation.status === 'pending_payment') {
          ...
          if (paymentStatus && kashierService.isPaymentSuccessful(paymentStatus.status)) {
            console.log('✅ Payment confirmed, creating RateHawk booking...');
            reservation.status = 'payment_confirmed';
            reservation.paymentConfirmedAt = new Date();
            await reservation.save();
